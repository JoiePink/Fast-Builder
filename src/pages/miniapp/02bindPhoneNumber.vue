...
const phoneCodeConfig = computed(() => JSON.stringify({
  executionOrder: [
    ...
    '按 authMode 解析登录注册接口并配置全部请求参数来源',
    '生成最终实现提示词',
  ],
...
const authApiReady = computed(() => {
  ...
  return roles.every(role => {
    ...
    return api && api.requestPool.every(param => param.sourceMode === 'pool' ? Boolean(param.sourceKey) : Boolean(param.customSource.trim()))
  })
})
...
function generatePrompt() {
  ...
  if (!authApiReady.value) {
    ElMessage.warning('请先解析登录注册接口并配置全部请求参数来源')
    return
  }
...
- 本阶段实现协议判断、手机号授权 code 入池、配置中选定的 code 使用分支，以及 getCode 获取 xcxCode；暂不实现登录、注册、token 等更后续逻辑。
...
- separate 模式参考 lawyer-huasheng：根据项目绑定状态选择登录或注册；若注册响应表示第三方用户已绑定，重新调用 wx.login 获取新的 xcxCode 后再登录。
- combined 模式参考 MrLove：手机号授权参数与 xcxCode 一并交给共用接口，不要额外调用独立注册接口。
- 每个认证接口请求参数必须按 sourceMode 取值：pool 从字段池 sourceKey 获取，custom 根据 customSource 的自然语言说明去目标项目中查找；不要凭空补值。
- 严格按 authApiInterfaces 中的 requestPool 组装请求，按 responsePool 读取返回结果。
...
function parseAuthApiDocument(role: 'login' | 'register' | 'combined') {
  ...
  try {
    ...
    const api: AuthApiInterface = {
      ...
      requestPool,
      responsePool: result.responseParams,
    }
    ...
    authApiStatuses[role] = `已解析 ${result.method} ${result.path}`
    generatedPrompt.value = ''
  }
  catch (error) {
...
function completeAuthApiStep() {
  if (!authApiReady.value) {
    ElMessage.warning('请解析所需接口，并为每个请求参数设置取值')
    return
  }
  advanceStep(5)
}
        ...
        <el-collapse-item v-for="api in authApiInterfaces" :key="api.id" :name="api.id">
          ...
          <el-table :data="api.requestPool" border>
            ...
            <el-table-column label="取值" min-width="280">
              <template #default="{ row }">
                <el-select v-if="row.sourceMode === 'pool'" v-model="row.sourceKey" filterable clearable placeholder="请选择字段池参数">
