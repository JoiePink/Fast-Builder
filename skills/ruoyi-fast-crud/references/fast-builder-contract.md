...
Fast-Builder 会输出一份共享的 `BuilderConfig` 和五份分阶段提示词。即使配置里包含完整 CRUD 信息，也必须只实现当前步骤。
...
- `formFields`：新增 / 修改表单字段。
  - `form.widget` 支持 `el-input`、`el-textarea`、`el-select`、`el-select-multiple`、`el-radio`、`el-date-picker`、`el-switch`、`image-upload`。
