...
- 实现 `step1_query_page` 的主表格时，不要盲目照搬 ApiFox/OpenAPI Schema 解析出的字段顺序；生成 `el-table` 列之前，必须结合 `businessName`、`pageName`、字段 `label` / `field`、目标项目相邻页面，对 `tableColumns` 做业务排序。
- 主表格字段排序优先级通常为：业务可识别主字段（名称、标题、编号、单号、用户/客户/商品/订单等核心信息）靠前，核心状态/类型/金额/数量/联系方式等常用业务字段居中，创建/更新时间、创建/更新人、备注等审计和补充字段靠后；`id`、内部标识、技术字段不要默认排在第一列，除非目标项目相邻页面就是这样。
- 实现 `step3_form` 时，不要盲目照搬 Schema 解析出的 `formFields` 顺序；生成新增 / 修改表单前，必须结合业务填写路径排序：核心基础信息靠前，业务属性和状态选项居中，图片/附件、排序、备注等补充字段靠后；主键、审计字段、内部标识通常不放在表单前面，除非用户明确启用且相邻页面如此。
- 实现 `step5_expand_row` 时，不要盲目照搬 Schema 解析出的 `expandFields` 顺序；生成 `el-descriptions` 前，必须结合业务阅读路径排序。若 `expandConfig.groups` 已有分组，保留分组边界和标题，只调整每个组内字段顺序；若是默认单组，则对全部展开字段统一排序。
